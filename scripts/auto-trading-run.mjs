...
const DEFAULT_API_URL = "http://localhost:3000/api/trading/run";
const DEFAULT_LOCK_PATH = join(process.cwd(), "data", "auto-trading.lock");
...
async function main() {
  ...
  if (config.mode === "off") {
    console.log(JSON.stringify({ status: "skipped", reason: "AUTO_TRADING_MODE is off." }));
    return;
  }
  if (config.mode === "live") {
    console.error(JSON.stringify({ status: "failed", reason: "Scheduled live trading is not supported." }));
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(
      JSON.stringify({
        ...
        planned: payload.summary?.plannedCount,
        blocked: payload.summary?.blockedCount,
        submitted: Array.isArray(payload.submissions)
          ? payload.submissions.filter((submission) => submission.status === "submitted").length
          : 0
      })
    );
  } finally {
...
function readConfig() {
  return {
    apiUrl: process.env.AUTO_TRADING_API_URL ?? DEFAULT_API_URL,
    mode: normalizeMode(process.env.AUTO_TRADING_MODE),
    symbols: parseSymbols(process.env.AUTO_TRADING_SCHEDULE_SYMBOLS ?? process.env.AUTO_TRADING_ALLOWED_SYMBOLS),
    lookbackDays: parseNumber(process.env.AUTO_TRADING_SCHEDULE_LOOKBACK_DAYS, 520),
    lockPath: process.env.AUTO_TRADING_LOCK_PATH ?? DEFAULT_LOCK_PATH,
    lockTtlMs: parseNumber(process.env.AUTO_TRADING_LOCK_TTL_MS, DEFAULT_LOCK_TTL_MS),
    skipClosedMarket: process.env.AUTO_TRADING_SKIP_MARKET_CLOSED !== "false"
  };
}
...
async function acquireLock(lockPath, ttlMs) {
  ...
  try {
    ...
    if (!isStaleLock(existingLock, ttlMs)) {
      console.error(JSON.stringify({ status: "skipped", reason: "Another auto-trading run is already active.", lockPath }));
      process.exitCode = 2;
...
function normalizeMode(value) {
  if (value === "paper" || value === "dry-run" || value === "off" || value === "live") {
    return value;
  }
